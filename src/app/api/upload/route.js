  ...
  try {
    ...
    if (!cloud || (!preset && (!apiKey || !apiSecret))) {
      return Response.json(
        {
          message:
            "Image upload is not configured. Add Cloudinary settings in Vercel Environment Variables.",
        },
        { status: 503 },
      );
    }
    ...
    if (!file || typeof file === "string")
      return Response.json({ message: "Select an image." }, { status: 400 });
    const allowedTypes = new Set(["image/jpeg", "image/png", "image/webp"]);
    if (!allowedTypes.has(file.type) || file.size > 5 * 1024 * 1024)
      return Response.json(
        { message: "Use a JPG, PNG or WebP image under 5 MB." },
        { status: 400 },
      );
    ...
    if (preset) {
      ...
      const signature = crypto
        .createHash("sha1")
        .update(
          `folder=advocate-vishnu/blogs&timestamp=${timestamp}${apiSecret}`,
        )
        .digest("hex");
    ...
    if (!response.ok || !data.secure_url)
      return Response.json(
        { message: data.error?.message || "Cloudinary image upload failed." },
        { status: 400 },
      );
    return Response.json({
      success: true,
