  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-md overflow-hidden rounded-3xl border border-white/10 bg-white p-7 text-center shadow-[0_30px_90px_rgba(15,23,42,.45)] sm:p-9">
        ...
        <div
          className={`mx-auto flex h-16 w-16 items-center justify-center rounded-full ${success ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}
        >
          {success ? (
            <FaCheck size={24} />
          ) : (
        ...
        <p
          className={`mt-5 text-xs font-black uppercase tracking-[0.2em] ${success ? "text-emerald-700" : "text-red-700"}`}
        >
          {success ? "Request Submitted" : "Submission Failed"}
        </p>
        <h2
        ...
        <button
          onClick={onClose}
          className={`mt-6 w-full rounded-xl px-5 py-3.5 text-sm font-bold text-white transition ${success ? "bg-emerald-700 hover:bg-emerald-800" : "bg-slate-950 hover:bg-slate-800"}`}
        >
